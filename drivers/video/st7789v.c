...
static const u8 init_data_vdvvrhen[] = {0x01};
static const u8 init_data_vrhs[] = {0x12};
static const u8 init_data_vdvs[] = {0x20};
static const u8 init_data_frctrl2[] = {0x0f};
static const u8 init_data_pwctrl1[] = {0xA4, 0xA1};
static const u8 init_data_pvgam[] = {0xD0,0x04,0x0D,0x11,0x13,0x2B,0x3F,0x54,0x4C,0x18,0x0D,0x0B,0x1F,0x23};
static const u8 init_data_nvgam[] = {0xD0,0x04,0x0C,0x11,0x13,0x2C,0x3F,0x44,0x51,0x2F,0x1F,0x1F,0x20,0x23};
...
static const struct st7789v_init_cmd st7789v_init_cmds[] = {
	...
	{0x21,NULL, 0, 0},
	{MIPI_DCS_EXIT_SLEEP_MODE,NULL, 0, 10},
...
{
	/*
	 * Use asserted/deasserted semantics so GPIO_ACTIVE_LOW works correctly.
	 * Typical reset: assert briefly, then deassert and wait for panel ready.
	 */
	st7789v_set_reset_asserted(dev, true);
	mdelay(ST7789V_RESET_DELAY_MS);
	st7789v_set_reset_asserted(dev, false);
...
static int st7789v_request_gpios(struct udevice *dev)
{
	...
	/* Optional backlight GPIO (many ST7789V modules need BL/LED=1 to light). */
	ret = gpio_request_by_name(dev, "backlight-gpios", 0, &priv->bl_gpio, GPIOD_IS_OUT);
	if (!ret) {
		priv->bl_requested = true;
		dm_gpio_set_value(&priv->bl_gpio, 1);
	} else {
		priv->bl_requested = false;
	}
...
static int st7789v_deferred_init(struct udevice *vid)
{
	...
	/*
	 * warm reset / 上电阶段，SPI 控制器可能尚未 probe（parent 未 active）。
	 * 如果这里直接放弃，会导致“概率性不亮屏”（一直等不到初始化发生）。
	 *
	 * 我们在 sync() 的递归保护上下文中尝试 probe 一次 parent：
	 * - 若 probe 失败：返回 -EAGAIN，后续 sync() 会继续重试
	 * - 若 probe 成功：继续正常初始化屏幕
	 */
	if (dev->parent && !device_active(dev->parent)) {
		serial_printf("Lois_debug: @st7789v.c device_probe parent not active\n");
		ret = device_probe(dev->parent);
		if (ret)
		serial_printf("Lois_debug: @st7789v.c device_probe parent not active: %d\n", ret);
			return -EAGAIN;
	}
	...
	if (!priv->gpio_requested) {
		...
		if (ret) {
			serial_printf("Lois_debug: @st7789v.c st7789v_request_gpios failed: %d\n", ret);
			return ret;
		}
	}
	...
	if (ret) {
		serial_printf("Lois_debug: @st7789v.c dm_spi_claim_bus failed: %d\n", ret);
		return ret;
	}

	ret = st7789v_init_display(dev);
	if (ret) {
		serial_printf("Lois_debug: @st7789v.c st7789v_init_display failed: %d\n", ret);
	...
	if (ret) {
		serial_printf("Lois_debug: @st7789v.c st7789v_set_madctl failed: %d\n", ret);
		dm_spi_release_bus(dev);
		return ret;
	}
	...
	dm_spi_release_bus(dev);
	if (ret) {
		serial_printf("Lois_debug: @st7789v.c st7789v_clear_screen failed: %d\n", ret);
		return ret;
	}
...
static int st7789v_sync(struct udevice *vid)
{
	...
	if (!priv->inited) {
		...
		{
			/*
			 * 对于 console 输出路径，初始化失败不应“卡住/中断启动流程”：
			 * -EAGAIN：父设备未就绪，稍后再试
			 * 其它错误：记录后跳过本次 sync，保持串口/启动继续
			 */
			serial_printf("Lois_debug: @st7789v.c st7789v_deferred_init not ready: %d\n", ret);
			syncing = false;
			return 0;
		}
	}
	...
	ret = dm_spi_claim_bus(dev);
	if (ret) {
		serial_printf("Lois_debug: @st7789v.c st7789v_sync Failed to claim SPI bus: %d\n", ret);
		syncing = false;
		return ret;
	}
...
static int st7789v_probe(struct udevice *dev)
{
	...
	if (!dev || !priv || !uc_priv) {
		serial_printf("Lois_debug: @st7789v.c st7789v_probe invalid parameters\n");
		return -EINVAL;
	}
	...
	serial_printf("Lois_debug: @st7789v.c st7789v_probe completed successfully\n");
	return 0;
}
